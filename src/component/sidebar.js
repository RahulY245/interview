...
export default function Sidebar() {
    const [cookies] = useCookies(["authToken", "role"]); // Access the cookies

    // Check if the token exists (indicating a logged-in user)
    const isLoggedIn = Boolean(cookies.authToken); // Check if authToken exists

    // Check the user type (admin or user)
    const isAdmin = cookies.role === "Admin"; // Check if the user is an admin
    const isUser = cookies.role === "User"; // Check if the user is a regular user
                        ...
                        <ul className="space-y-2 font-medium">
                            {isAdmin && (
                                <li>
                                    ...
                                    >
                                        <svg
                                            className="w-5 h-5 text-gray-200 transition duration-75 dark:text-gray-400 group-hover:text-gray-50 dark:group-hover:text-white"
                                            aria-hidden="true"
